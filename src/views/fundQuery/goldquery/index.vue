...
<script>
import {goldList} from '@/api/fundQuery'
 ...
 methods: {
      //  渲染table列表
       getList(){
        ...
        goldList(that.listQuery).then(response=>{
             that.goldList = response.data.items
     ...
     handleFilter() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const data = this.formatJson(filterVal, this.goldList)
