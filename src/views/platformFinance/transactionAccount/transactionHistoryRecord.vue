<template>
    <div class="historyrecord-container">
      ...
      <el-table-column label="币种" align="center" min-width="100px">
       ...
       <el-table-column label="剩余账户资产" min-width="90px" align="center">
            <template slot-scope="{row}">
                 <span>{{row.sheets_number}}</span>
            </template>
      </el-table-column>
        <el-table-column label="剩余已实现盈亏" min-width="90px" align="center">
            <template slot-scope="{row}">
                 <span>{{row.sheet_number}}</span>
            </template>
      </el-table-column>
...
<script>
import {transactionHistoryRecord} from '@/api/platformFinance'
 ...
 methods: {
     getList(){
        ...
        that.listLoading = true
         transactionHistoryRecord(that.listQuery).then(response=>{
            //  console.log(response)
               that.tranctionHistoryList = response.data.items
             that.total = response.data.total
             console.log(that.tranctionHistoryList)
            // 过了1.5秒就关闭
               setTimeout(() => {
